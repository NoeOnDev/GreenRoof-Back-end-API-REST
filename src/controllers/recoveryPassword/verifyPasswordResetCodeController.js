...
async function verifyPasswordResetCodeController(req, res) {
  try {
    ...
    if (user.verificationCode !== verificationCode) {
      return res
        .status(400)
        .json({ error: "El código de verificación no es válido" });
    }
    ...
    res.status(500).json({
      error:
        "Ocurrió un error al verificar el código. Por favor, inténtelo de nuevo más tarde.",
    });
  }
}
...
async function clearVerificationCode(email) {
  return new Promise((resolve, reject) => {
    connection.query(
      "UPDATE users SET verificationCode = NULL WHERE email = ?",
      [email],
      (err, result) => {
        if (err) {
