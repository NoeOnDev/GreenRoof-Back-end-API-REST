...
async function loginUserController(req, res) {
  try {
    ...
    if (!user) {
      return res
        .status(401)
        .json({ error: "No hemos podido autenticar su acceso" });
    }
    ...
    if (!isRecaptchaValid) {
      return res
        .status(400)
        .json({ error: "Completa el reCAPTCHA para continuar" });
    }
    ...
    if (!user.verified) {
      return res
        .status(401)
        .json({
          error: "Por favor, verifique su cuenta antes de iniciar sesión",
        });
    }
    ...
    res
      .status(200)
      .json({
        message: "Datos de acceso validados satisfactoriamente",
        token,
    ...
    res
      ...
      .json({
        error:
          "Ocurrió un error durante el proceso de inicio de sesión. Por favor, inténtelo de nuevo más tarde.",
      });
  }
}
