...
async function verifyUserController(req, res) {
  try {
    ...
    const updateQuery =
      "UPDATE users SET verified = true, verificationToken = NULL WHERE verificationToken = ?";
    const result = await queryDatabase(updateQuery, [token]);

    if (result.affectedRows === 0) {
      const invalidTokenMessage =
        await verifyInvalidService.generateInvalidTokenMessage();
      return res.send(invalidTokenMessage);
    } else {
      const verificationMessage =
        await verifyValidService.generateVerificationMessage();
    ...
    res.status(500).json({
      error:
        "Ocurrió un error durante la verificación de correo. Por favor, inténtelo de nuevo más tarde.",
    });
  }
}
