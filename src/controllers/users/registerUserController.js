  ...
  try {
    ...
    if (!email || !username || !password) {
      return res
        ...
        .json({
          error:
            "Por favor complete todos los campos requeridos antes de registrarse",
        });
    }
    ...
    if (results.length > 0) {
      if (results[0].verified) {
        return res
          .status(400)
          .json({ error: "Este correo ya está registrado y verificado" });
      } else {
        const { verificationToken, hashedPassword } = await handleVerification(
          email,
          password,
          username
        );
        ...
        return res
          .status(200)
          .json({ message: "Su registro ha sido actualizado con éxito" });
      }
    }
    ...
    res
      ...
      .json({
        error:
          "Ocurrió un error durante el proceso de registro. Por favor, inténtelo de nuevo más tarde.",
      });
  }
}
...
async function updateUser(email, username, hashedPassword, verificationToken) {
  const updateQuery =
    "UPDATE users SET username = ?, password = ?, verificationToken = ? WHERE email = ?";
...
async function insertUser(email, username, hashedPassword, verificationToken) {
  const insertQuery =
    "INSERT INTO users (email, username, password, verificationToken, verified) VALUES (?, ?, ?, ?, ?)";
  const insertValues = [
    email,
    username,
    hashedPassword,
